...
class _AnasayfaState extends State<Anasayfa> {
  ...
  Future<void> test() async {
    ...
    sp.setStringList("liste", arkadasListe);

    //Silme
    //sp.remove("ad");

    //Okuma
    String gelenAd = sp.getString("ad") ?? "isim yok"; // ?? -> varsayılan değer
    ...
    var gelenListe = sp.getStringList("liste");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Text("Açılış sayısı : $sayac",style: TextStyle(fontSize: 40),),
      ),
    );
  }
}
